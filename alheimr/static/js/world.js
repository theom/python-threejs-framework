...
alheimr.world.prototype.start = function start()
{
    this.init_listeners();
...
alheimr.world.prototype.init_listeners = function init_listeners()
{
...
alheimr.world.prototype.create_renderer = function create_renderer()
{
...
alheimr.world.prototype.create_scene = function create_scene(set)
{
    var scene_set = {};
...
alheimr.world.prototype.create_camera = function create_camera(set)
{
...
alheimr.world.prototype.create_hud_camera = function create_hud_camera(set)
{
    var w = this.canvas.clientWidth;
    var h = this.canvas.clientHeight;
    var far_plane = 2000;
...
alheimr.world.prototype.create_lights = function create_lights()
{
    var light;
...
alheimr.world.prototype.init_window_resize = function init_window_resize()
{
    var self = this;
    window.addEventListener("resize", function(event) { self.on_window_resize.call(self, event); }, false);
}

alheimr.world.prototype.init_picking = function init_picking()
{
...
alheimr.world.prototype.init_keyboard = function init_keyboard()
{
    var self = this;
...
alheimr.world.prototype.connect_to_server = function connect_to_server()
{
    var self = this;
    try
    {
        this.socket = new alheimr.socket("ws://" + location.host + "/ws");
    ...
    catch (e)
    {
        if (this.debug)
        {
            console.debug("alheimr.world", "connect_to_server");
        }
    }
}

alheimr.world.prototype.on_socket_open = function on_socket_open()
{
...
alheimr.world.prototype.on_socket_message = function on_socket_message(event)
{
...
alheimr.world.prototype.on_socket_close = function on_socket_close()
{
    console.log("socket closed");
}

alheimr.world.prototype.on_socket_error = function on_socket_error(error)
{
    console.log("socket error: '" + error + "'");
}

alheimr.world.prototype.register_event_listener = function register_event_listener(event, object, handler)
{
    var listener = {};
...
alheimr.world.prototype.on_window_resize = function on_window_resize(event)
{
    this.renderer.setSize(this.canvas.clientWidth, this.canvas.clientHeight);
...
alheimr.world.prototype.on_mouse_move = function on_mouse_move(event)
{
...
alheimr.world.prototype.scene_picking = function scene_picking(scene_set)
{
...
alheimr.world.prototype.on_mouse_down = function on_mouse_down(event)
{
...
alheimr.world.prototype.on_mouse_out = function on_mouse_out(event)
{
...
alheimr.world.prototype.on_mouse_up = function on_mouse_up(event)
{
    this.left_mouse_down = false;
}

alheimr.world.prototype.on_key_press = function on_key_press(event)
{
    for (var i = 0; i < this.scene_sets.length; i++)
    {
        var scene_set = this.scene_sets[i];
...
alheimr.world.prototype.on_key_down = function on_key_down(event)
{
    for (var i = 0; i < this.scene_sets.length; i++)
    {
        var scene_set = this.scene_sets[i];
...
alheimr.world.prototype.on_key_up = function on_key_up(event)
{
    for (var i = 0; i < this.scene_sets.length; i++)
    {
        var scene_set = this.scene_sets[i];
...
alheimr.world.prototype.register_entity = function register_entity(ent)
{
    this.entities[ent.name] = ent;
}

alheimr.world.prototype.register_component = function register_component(comp)
{
    this.components.push(comp);
}

alheimr.world.prototype.create_world = function create_scene()
{
    var tests = new alheimr.tests(this, this.scene_sets[0].scene, this.scene_sets[1].scene);
    ...
    // File upload
    var fu = new alheimr.file_upload("Upload image");
    var self = this;
    fu.on_load = function(event)
    {
        var msg = {};
        msg.to = "alheimr.images";
        msg.from = this.name;
    ...
    this.image = new alheimr.image(200);
...
alheimr.world.prototype.display_image = function display_image(img_base64)
{
    this.image.set_image(img_base64);
}

alheimr.world.prototype.render = function render()
{
    var self = this;
    setTimeout(function()
               {
...
alheimr.world.prototype.update = function update_scene()
{
...
alheimr.world.prototype.set_name = function set_name(name)
{
    if (this.name_set)
    {
        return;
    ...
    var msg = {};
    msg.to = "alheimr.registry";
    msg.from = this.name;
    msg.payload = this.name + " confirmed";
    this.send_to_server(msg);
}

alheimr.world.prototype.start_entities = function start_entities()
{
...
alheimr.world.prototype.deliver_messages = function deliver_messages()
{
...
alheimr.world.prototype.parse_message = function parse_message(msg)
{
    var new_msg = {};
    var done = false;
    var words = msg.split(" ");
...
alheimr.world.prototype.send = function send(msg)
{
...
alheimr.world.prototype.send_to_server = function send_to_server(msg)
{
...
alheimr.world.prototype.construct_components_loader = function construct_components_loader()
{
    ...
    var one_shot = new alheimr.one_shot(this, domain + "trigger");
    one_shot.connect_receiver(domain + "dist");
    this.register_entity(one_shot);
    
    var dist = new alheimr.distributor(this, domain + "dist");
    this.register_entity(dist);

    var sync_name = domain + "sync";
    var sync = new alheimr.synchronizer(this, sync_name);
    ...
    var caller = new alheimr.caller(this, caller_name);
    caller.set_method(this, this.post_load);
    this.register_entity(caller);
}

alheimr.world.prototype.create_loader = function create_loader(name, sync, code)
{
    var ldr = new alheimr.code_loader(this, name);
    ldr.load(code);
    ldr.connect_receiver(sync);
    this.register_entity(ldr);
}

alheimr.world.prototype.post_load = function post_load()
{
    this.create_world();
    this.render();
}
